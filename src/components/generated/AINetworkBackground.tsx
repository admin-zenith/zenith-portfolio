...
export const AINetworkBackground = () => {
  return <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <motion.div className="absolute bottom-1/4 left-1/2 w-80 h-80 bg-cyan-500/10 rounded-full blur-3xl" animate={{
      x: [0, -40, 0],
      y: [0, -20, 0],
      scale: [1, 1.3, 1]
    }} transition={{
      ...
      <svg className="absolute inset-0 w-full h-full opacity-30">
        ...
        {/* Flowing curves */}
        <motion.path d="M0,100 Q250,50 500,100 T1000,100" stroke="url(#lineGradient1)" strokeWidth="2" fill="none" initial={{
        pathLength: 0
      }} animate={{
        pathLength: [0, 1, 0]
      }} transition={{
        ...
        <motion.path d="M1000,200 Q750,250 500,200 T0,200" stroke="url(#lineGradient1)" strokeWidth="2" fill="none" initial={{
        pathLength: 0
      }} animate={{
        pathLength: [0, 1, 0]
      }} transition={{
      ...
      {Array.from({
      length: 15
    }).map((_, i) => <motion.div key={i} className="absolute w-1 h-1 bg-[#c2f12d] rounded-full opacity-40" style={{
